...
import (
	...
	"github.com/mothdb-bd/common-go/pkg/maths"
)
...
func CopyBools(from []bool, srcPos int32, dest []bool, destPos int, length int32) {
	n := 0
	for i := srcPos; i < (srcPos+length) && int(i) < len(from); i++ {
...
func CopyOfBytes(original []byte, newLength int32) []byte {
	c := make([]byte, newLength)
	ol := len(original)
	copy(c[0:maths.MinInt32(int32(ol), newLength)], original[0:maths.MinInt32(int32(ol), newLength)])
	return c
}

func CopyInt8s(from []int8, srcPos int32, dest []int8, destPos int32, length int32) {
	n := INT32_ZERO
	for i := srcPos; i < (srcPos+length) && int(i) < len(from); i++ {
		dest[n+destPos] = from[i]
		n++
...
func CopyOfInt8s(original []int8, newLength int32) []int8 {
	copy := make([]int8, newLength)
	ol := len(original)
	CopyInt8s(original, 0, copy, 0,
		maths.MinInt32(int32(ol), newLength))
	return copy
}

func CopyInt8sOfRange(original []int8, from int32, to int32) []int8 {
	newLength := to - from
	if newLength < 0 {
		panic(strconv.Itoa(int(from)) + " > " + strconv.Itoa(int(to)))
	}
	newLength = maths.MinInt32(int32(len(original)), newLength)
	copy := make([]int8, newLength)
	CopyInt8s(original, from, copy, 0, newLength)
	return copy
}
