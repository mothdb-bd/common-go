...
// 指定list的size大小返回arraylist
func NewListBySize[T basic.Object](size int32) *ArrayList[T] {
	list := &ArrayList[T]{}
	list.elements = make([]T, size)
	return list
}
...
func (list *ArrayList[T]) GetByUInt8(index uint8) T {
	return list.Get(int(index))
}

func (list *ArrayList[T]) GetByUInt16(index uint16) T {
	return list.Get(int(index))
}
...
func (list *ArrayList[T]) SizeUInt8() uint8 {
	return uint8(list.size)
}
